...
/** 내 리뷰·찜 API. 소유자는 서버의 인증 정보로 결정하고 사용자 ID를 보내지 않는다. */
...
function resourceId(value) {
  ...
  if (!/^[1-9]\d*$/.test(id)) throw new Error('올바른 장소 또는 리뷰 ID가 필요합니다.')
  return id
}
...
export function listFavorites({ page = 0, size = 20, sort = 'recent' } = {}) {
  const query = new URLSearchParams({ page, size, sort })
  return apiRequest(`/users/me/favorites?${query}`, authenticated)
}

export function getFavorite(placeId) {
  return apiRequest(`/users/me/favorites/${resourceId(placeId)}`, authenticated)
}

export function setFavorite(placeId, selected) {
  return apiRequest(`/users/me/favorites/${resourceId(placeId)}`, {
    ...authenticated, method: selected ? 'PUT' : 'DELETE',
  })
}
